Feature: the export endpoint handles template creation
    ...
    Scenario: client makes call to GET /export/template/TemplateExport:not-existing and receives an 404 response
        Given database app is created
        Given signed in as client dummy-client with secret client-secret in realm dummy-realm
        When the client calls GET /v1/export/template/TemplateExport:not-existing
