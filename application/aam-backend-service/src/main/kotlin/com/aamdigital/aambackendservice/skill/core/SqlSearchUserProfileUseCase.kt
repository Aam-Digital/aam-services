...
class SqlSearchUserProfileUseCase(
    ...
    override fun apply(request: SearchUserProfileRequest): UseCaseOutcome<SearchUserProfileData> {
        ...
        if (!userProfile.email.isNullOrBlank()) {
            searchResults =
                userProfileRepository.findAll(
                    Example.of(userProfile, MATCHER_EMAIL),
                    pageable
                )
        ...
        if (!userProfile.mobileNumber.isNullOrBlank()) {
            searchResults =
                userProfileRepository.findAll(
                    Example.of(userProfile, MATCHER_PHONE),
                    pageable
                )
        ...
        // check for name
        searchResults =
            if (!userProfile.fullName.isNullOrBlank()) {
                userProfileRepository.findAll(
                    Example.of(userProfile, MATCHER_NAME),
                    pageable
                )
            } else {
                Page.empty()
    ...
    ): UseCaseOutcome.Success<SearchUserProfileData> =
